  ...
  selectIcon(type: number) {
    switch (type) {
      case 1:
        return (
          <svg
            className="race__icon--type"
            data-testid="icon-thoroughbred"
            viewBox="0 0 32 32"
          >
      ...
      <AppContext.Consumer>
        {(props: any) => {
          return (
            <React.Fragment>
              <div>
                {Object.values(props.current).map((item: any, index) => (
                  <React.Fragment key={"rc-item-frg-" + index}>
                    <div
                      key={"rc-item-" + index}
                      className="race__event"
                      data-testid={"race-item-" + index}
                    >
                      <a href="/" key={"rc-item-anchor-" + index}>
                        <div key={"rc-item-EventId-" + index}>
                          {this.selectIcon(item.EventType.EventTypeID)}
                        </div>
                        <div
                          key={"rc-item-eDetails-" + index}
                          className="race__event--detail"
                        >
                          <div
                            key={"rc-item-venue-" + index}
                            data-testid={"race-venue-" + index}
                            className="race__venue"
                          >
                            {item.Venue.Venue}
                          </div>
                          <div
                            className="race__rc"
                            key={"rc-item-eName-" + index}
                          >
                            <span key={"rc-item-eName-text-" + index}>
                              {item.EventName}
                            </span>
                          </div>
                        </div>
                        <div
                          key={"rc-item-timer-" + index}
                          className="race__timer--tick"
                        >
                          <span
                            className="start__timer start__left"
                            key={"rc-item-timer-count-" + index}
                          >
                            {new Date(
                              item.AdvertisedStartTime
                            ).getUTCMinutes() +
                              "m " +
                              new Date(
                                item.AdvertisedStartTime
